...
$js = <<<JS
    ...
    $(document).delegate('.body-valor .open-children .fa-plus', 'click', function()
    {
    ...
    $(document).delegate('.body-valor .open-children .fa-minus', 'click', function()
    {
        var _id = $(this).parent().parent().data('id');
    ...
    $('.body-valor .i-alterar').click(function () 
    {
        var _id = $(this).data('id');
        var url = '/balancete/alterar-valor?id=' + _id;
        $('#iframe_modal_balancete').attr('src', url);
        $("[id^='modal_balancete']").modal("show");
        $('.modal-backdrop').removeClass('modal-backdrop');
        $('#modal_balancete .modal-header h3').text('Alterar valor');
    });
        
    $('.body-valor .i-inserir').click(function () 
    {
        var _balanceteid = $(this).data('balanceteid');
        var _categoriaid = $(this).data('categoriaid');
        var url = '/balancete/inserir-valor?balancete_id=' + _balanceteid + '&categoria_id=' + _categoriaid;
        $('#iframe_modal_balancete').attr('src', url);
        $("[id^='modal_balancete']").modal("show");
        $('.modal-backdrop').removeClass('modal-backdrop');
        $('#modal_balancete .modal-header h3').text('Alterar valor');
                    ...
                    <?php $sumbc = 0; foreach($bb["children"] as $ibc =>  $bc): ?>
                        ...
                        <?php $sumbd = 0; foreach($bc["children"] as $ibd =>  $bd): ?>
                            ...
                            $js = <<<JS
                                ...
                                $('.body-valor .val-{$ibd}-{$ibc}-{$ibb}-{$iba}-{$ias}').html('{$valbe}');
                                $('.body-valor .val-{$ibc}-{$ibb}-{$iba}-{$ias}').html('{$valbd}');
                                $('.body-valor .val-{$ibb}-{$iba}-{$ias}').html('{$valbc}');
                                $('.body-valor .val-{$iba}-{$ias}').html('{$valbb}');
                                $('.body-valor .val-{$ias}').html('{$valba}');
